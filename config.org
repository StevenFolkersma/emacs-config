...
This file describes my emacs configuration. Feel free to copy parts of the
configuration, I do not recommend copying it in full as it contains many
personal functions that will mostly likely not work for everyone. The config
is heavily inspired by (and parts are copied from) the configs of Prot,
karthinks and oantolin.
...
** The =setup-basics.el= with simple convenience commands

Quality of life snipper from Prot's setup.
#+begin_src emacs-lisp :tangle "steven/setup-basic.el"
;;; -*- lexical-binding: t -*-
...
** The =text-extras.el= section on text stuff and semantic lins

The semantic line switching code is copied from: [[https://sachachua.com/blog/2025/09/emacs-cycle-through-different-paragraph-formats-all-on-one-line-wrapped-max-one-sentence-per-line-one-sentence-per-line/][Emacs: Cycle through different paragraph formats: all on one line, wrapped, m...]]

I also added a minor-mode to switch to One Sentence Per Visual Line for the whole buffer.
#+begin_src emacs-lisp :tangle "steven/text-extras.el"
;;; -*- lexical-binding: t -*-
...
#+begin_src emacs-lisp :tangle "steven/theme-extras.el"
;;; -*- lexical-binding: t -*-
...
** The =init.el= section on trying out keycast
...
modeline have /no/ box. This also holds for the headerline.

This is a setup using mode-line-maker of Nicolas Rougier (link)
#+begin_src emacs-lisp :tangle "steven/setup-modeline.el"
;;; -*- lexical-binding: t -*-
...
** =init= on loading modeline with use-package
...
** The =init.el= section for setting up project.el
...
** =The =init.el= with custom ffap-menu function

#+begin_src emacs-lisp 
;;; ffap-menu-ext.el --- Extensible ffap-menu  -*- lexical-binding:t; -*-

(require 'ffap)
(require 'subr-x)

(defgroup ffap-menu-ext nil
  "Extensible ffap-menu."
  :group 'ffap)

(defcustom ffap-menu-collector-functions
  '(ffap-menu-collect-regexp)
  "Functions used to collect ffap-menu candidates.

Each function is called with no arguments in the current buffer and
returns a list of (DISPLAY . TARGET) pairs."
  :type 'hook)

(defun ffap-menu--collect ()
  "Collect candidates from all collectors."
  (delete-dups
   (apply #'append
          (mapcar #'funcall ffap-menu-collector-functions))))

;;; Default collector

(defun ffap-menu-collect-regexp ()
  "Collect URLs and file names using `ffap-menu-regexp'."
  (let (result)
    (save-excursion
      (goto-char (point-min))
      (while (re-search-forward ffap-menu-regexp nil t)
        (let ((thing (match-string-no-properties 0)))
          (push (cons thing thing) result))))
    (nreverse result)))

;;; SHR collector

(defun ffap-menu-collect-shr-links ()
  "Collect links stored in the `shr-url' text property."
  (let (result)
    (save-excursion
      (goto-char (point-min))
      (while (< (point) (point-max))
...
(with-eval-after-load 'denote
  ...
  (defun ffap-menu-collect-denote-links ()
    "Collect Denote links in the current buffer."
    (let (result)
      (save-excursion
        (goto-char (point-min))
        (while (re-search-forward "denote:\\([[:alnum:]_-]+\\)" nil t)
          (when-let* ((id (match-string-no-properties 1))
                      (file (ignore-errors
                              (denote-get-path-by-id id))))
            (push (cons (format "denote:%s" id)
                        file)
                  result))))
      (nreverse result)))

  (add-to-list 'ffap-menu-collector-functions
               #'ffap-menu-collect-denote-links))

(add-to-list 'ffap-menu-collector-functions
             #'ffap-menu-collect-shr-links)

;;; Prototype command

(defun ffap-menu-ext ()
  "Like `ffap-menu', but supports extensible collectors."
  (interactive)
  (let* ((candidates (ffap-menu--collect))
         (choice
          (completing-read
           "Find: "
...
;(provide 'ffap-menu-ext)

;;; ffap-menu-ext.el ends here
#+end_src
...
** The =init.el= section on dwim-shell-command and image-extras
#+begin_src emacs-lisp :tangle "init.el"
...
** The =image-extras.el= section defining image helpers

#+begin_src emacs-lisp :tangle "steven/image-extras.el"
;;; -*- lexical-binding: t -*-
...
** TODO This =init.el= section on languages 
...
#+begin_src emacs-lisp :tangle "steven/setup-language.el"
;;; -*- lexical-binding: t -*-
...
** TODO The =init.el= setup of the dictionary
...
** The =init.el= section to setup spell-checking with jinx

Jinx is a fast spell-checker that uses =enchant-2= as its backend,
which in turn uses hunspell. Both =en_GB= and =nl_NL= dictionaries are
installed in =/usr/share/hunspell=. The language toggle in
=setup-language.el= updates =jinx-languages= and reloads the checker.
...
Key bindings:
- =<f7>= :: toggle jinx-mode on/off
- =M-$= :: correct word at point (Vertico picker with candidates)
- =C-;= :: quick fix word at/before point (Vertico picker, Enter accepts top suggestion)
- =C-M-$= :: correct all misspellings in buffer
- =C-<f7>= :: toggle language between English and Dutch
...
** TODO The =init.el= to relocate auto-save files
...
** TODO The =init.el= section on repeat mode
...
This are experiments, function copied from Rougier in GTD guide, but I do not
use it. Note block are a funny idea, and in general maybe styling source
blocks could be fun.
#+begin_src emacs-lisp
...
Requires ImageMagick. See full custom function in [[*The =setup-export.el= section on exporting tools]]
...
Publishing function for my recipe website. Table of Contents are removed for the entire website here using *:with-toc nil* , is stored in 
...
#+begin_src emacs-lisp :tangle "personal/mkdocs-extras.el"
;;; -*- lexical-binding: t -*-
...
(defun steven-recipebook-mkdocs-build ()
  ...
  (interactive)
  (let ((default-directory "~/Documents/Projects/website-recipes/"))
    (async-shell-command
...
(provide 'mkdocs-extras)
;;mkdocs-extras.el ends here
#+end_src
...
** The =setup-export.el= section on exporting tools 

#+begin_src emacs-lisp :tangle "personal/setup-export.el"
;;; -*- lexical-binding: t -*-
...
(setq org-publish-project-alist
      '(("recipebook-recipes"
         :base-directory "~/Documents/Recipebook/Recipes"
         :publishing-directory "~/Documents/Projects/website-recipes/docs/Maaltijden"
         :recursive t
         :with-toc nil
         :publishing-function org-md-publish-to-md)
        ("recipebook-images"
         :base-directory "~/Documents/Recipebook/Recipes/images"
         :publishing-directory "~/Documents/Projects/website-recipes/docs/Maaltijden/images"
         :recursive t
         :base-extension "png\\|jpg\\|gif\\|jpeg"
         :publishing-function org-publish-attachment-resize)
        ("recipebook" :components ("recipebook-recipes" "recipebook-images"))))
...
% Remove section numbering if desired
...
;; Resizing function used in recipebook website
...
(provide 'setup-export)
;;setup-export ends here
#+end_src
...
(use-package consult-omni
  :vc (:url "https://github.com/armindarvish/consult-omni" :rev :newest)
  :after consult
  :bind (:map search-map
         ("w" . consult-omni-multi)
         ("W" . consult-omni-wikipedia))
  :config
  (add-to-list 'load-path
               (expand-file-name "sources"
                                 (file-name-directory (locate-library "consult-omni"))))
  (require 'consult-omni-wikipedia)
  (require 'consult-omni-brave)
  (setq consult-omni-multi-sources '("Wikipedia" "Brave"))
  (setq consult-omni-brave-api-key
        (lambda ()
          (auth-source-pick-first-password :host "api.search.brave.com" :user "apikey")))
...
** The =init.el= section on prot pair
#+begin_src emacs-lisp :tangle "init.el"
(require 'prot-pair)
...
** The =init.el= section to setup completion with vertico
...
** The =frame-extras.el= section on frame commands

#+begin_src emacs-lisp :tangle "steven/frame-extras.el"
;;; -*- lexical-binding: t -*-
...
#+begin_src emacs-lisp :tangle "personal/email-extras.el"
...
#+begin_src emacs-lisp :tangle "steven/steven-magit-extras.el"
;;; -*- lexical-binding: t -*-
...
** The =init.el= sectioon on the setup of latex and pdf
...
#+begin_src emacs-lisp :tangle "steven/pdf-extras.el"
;;; -*- lexical-binding: t -*-
...
** The =gtpel-extras.el= section nfor gptel extras
...
** The =socratic-extras.el= section for Socratic note questioning

Sends the current note (or region) to the Claude Code CLI with a Socratic system
prompt and a personal context file. Returns 3–5 probing questions into a
=*Socratic Questions*= buffer. Edit =prompts/my-context.txt= to keep the context
file current.

#+begin_src emacs-lisp :tangle “init.el”
(use-package socratic-extras
  :bind ((“C-c a q” . steven-socratic-current-note)
         (“C-c a Q” . steven-socratic-region)))
#+end_src
...
#+begin_src emacs-lisp :tangle "steven/center-document-mode.el"
;;; -*- lexical-binding: t -*-
...
** The =init.el= section to load whisper.el

Trying out speech to text inside emacs.
...
** The =init.el= section to load kobo-markings
...
** The =embark-extras.el= section with custom embark functions
...
Now loading the package from git, [[*The =init.el= section to set up denote-embark]]
...
** The =steven-elfeed-extras.el= section with custom elfeed module

#+begin_src emacs-lisp :tangle "steven/steven-elfeed-extras.el"
;;; -*- lexical-binding: t -*-
(require 'elfeed)
